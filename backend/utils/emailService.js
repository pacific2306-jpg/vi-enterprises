...
const sendInquiryEmails = async (inquiryData) => {
  ...
  const customerHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; border-top: 4px solid #1e3a8a; padding: 32px; color: #334155; line-height: 1.6;">
      ...
      <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
      <p style="font-size: 13px; color: #94a3b8; margin: 0;"><strong>VI Enterprises — Industrial Plating & Surface Finishing</strong><br />
      W-101, M.I.D.C Industrial Area, Hingna Rd, Nagpur, Maharashtra 441110</p>
    </div>
  `;

  // 3. Build the payload for Brevo's HTTPS REST API (Completely bypasses firewalls and domain locks)
  const brevoData = {
    sender: { name: "VI Enterprises", email: "pacific2306@gmail.com" }, // Sent from your Gmail alias safely
    to: [
      { email: "pacific2306@gmail.com", name: "Sales Desk" }, // Send lead to you
      { email: email, name: name } // Send thank you directly to the visitor's typed email!
    ],
    messageVersions: [
      {
        to: [{ email: "pacific2306@gmail.com", name: "Sales Desk" }],
        subject: `🚨 NEW INDUSTRIAL INQUIRY: ${requirement} from ${companyName}`,
  ...
  // 4. Send over standard secure HTTPS web port 443
